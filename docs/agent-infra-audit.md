...
- [x] **DONE (PR #146), corrected 2026-07-02.** DB default model string + Alembic migration (`as01f5b7c9d0`) → `claude-sonnet-5`; single head preserved. **Correction:** the `ai_insights` table this migration altered is legacy — the initial schema (`550a40162883`) drops it in `upgrade()` (the audit's original citation pointed at that migration's `downgrade()` section), so no current database has it and there is no live model-default column to bump. The unconditional ALTER failed the first post-billing-unblock staging release command (`relation "ai_insights" does not exist`), blocking the migration chain; the migration is now guarded on table existence and applies as a recorded no-op.
...
- [x] **DONE (follow-up PR).** Extend failover to the Batches path. `run_batch` now owns the submit → poll → per-entry failover → merge lifecycle: entries that come back errored with a *retryable* reason (transient overload/timeout via `overloaded_error`/`api_error`/`rate_limit_error`/`timeout_error`, or a model that was unavailable via `not_found_error`) are resubmitted once on the fallback tier (one step down, never up), and successful retries overwrite the errored result; deterministic client errors (`invalid_request_error` etc.) are left as-is. The submit call itself got bounded transient retry (`_create_batch`). Batch entries now share the live path's request shaping (`_build_create_kwargs`), so they inherit the temperature guard **and** Sonnet-5 thinking suppression (a latent gap — the old hand-built batch params never disabled adaptive thinking). Falls back to sequential `ainvoke` (its own retry+failover) when the SDK lacks the Batches surface. Retryable-error classification is centralized in `llm_client.batch_error_is_retryable`. **All audit items now closed.**
  **Hardening follow-up:** the failover round is best-effort (its own submit/poll failure returns first-round results instead of discarding paid work); every submitted `custom_id` is guaranteed present in the result (poll timeout / fetch failure surfaces as `error type "result_unavailable"`, deliberately non-retryable since the original batch may still complete server-side); successful batch entries record `llm_call_telemetry` via `call_site`, same as the live path.
  **Adoption:** the nightly tenant rollup (`Orchestrator.run_daily`) now submits all of a tenant's entity-consolidation calls (Opus, 10–50 per tenant) through `run_batch` — one Batches round-trip per tenant inside the existing per-tenant chord, ~50 % token discount, per-entry failover, and `orchestrator_daily` telemetry via `call_site`. Failure semantics per entity are unchanged: a failed entry (batch error / poll timeout / submit failure) leaves its deltas unconsumed for the next nightly run. Remaining `invoke`/`ainvoke` surfaces (weekly reflection, manager recommendations, KB refiners — single call per tenant or Haiku-tier) are lower-value batch candidates, still open.
