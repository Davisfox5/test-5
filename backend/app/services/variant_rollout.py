...
def _stats_for_variant(
    ...
    """(n, mean, variance) of quality scores for one variant.

    Pulls the score column and aggregates in Python — sample sizes are
    experiment-scale (10²–10³) and this stays portable across dialects.
    """
    values = [
        float(s)
        for (s,) in (
...
def evaluate_active_experiments(session: Session) -> Dict[str, Any]:
    ...
    for exp in experiments:
        if not exp.control_variant_id or not exp.treatment_variant_id:
            continue
        c_n, c_avg, c_var = _stats_for_variant(session, exp.control_variant_id, exp.start_date)
        t_n, t_avg, t_var = _stats_for_variant(session, exp.treatment_variant_id, exp.start_date)
