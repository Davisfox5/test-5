...
ANALYSIS_SYSTEM_PROMPT = (
    ...
    "buckets.\n"
    "- concerns_raised: list of {topic: str, description: str, severity: "
    "'high'|'medium'|'low', sentiment: 'positive'|'neutral'|'negative'|"
    "'mixed', quote: str|null} — every pain-point, worry, or risk the "
    "customer mentioned in this call. ``topic`` is a short canonical "
    "string (``pricing``, ``data_security``, ``integration_xyz``); we "
    "deduplicate concerns across the customer's lifetime by this string, "
    "so consistent topic naming matters. ``sentiment`` captures whether "
    "the concern was raised hot (negative), is being monitored (positive "
    "= they feel better about it), or just acknowledged (neutral). When "
    "the customer didn't raise any concerns, return an empty list — "
    "don't pad.\n"
    "- customer_commitments: list of {description: str, quote: str|null, "
    "due_date: 'YYYY-MM-DD'|null} — commitments the CUSTOMER made to us "
    "(``\"we'll get you the contract by Friday\"``). The rep-side "
    "commitments belong in action_items as usual; this list is the "
    "symmetric mirror. ``due_date`` only when explicit. Empty list when "
    "the customer made no commitments.\n\n"
