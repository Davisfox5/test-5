...
ACTION_PLAN_PROMPT_VERSION = "2026-06-02.critical-slots-lazy-draft"
...
_VOICE_RULES = (
    ...
    "'X rather than Y' / 'X instead of Y' / 'X (em-dash) not Y' "
    "constructions to score a point. They read as AI ad-copy. "
    "BAD: 'the right questions, not assumed ones'; 'an ally, not "
    "a skeptic'; 'a document, not a call'. GOOD: state the "
    "positive assertion directly ('lets Thursday focus on the real "
    "evaluation criteria'; 'helps the senior dispatcher arrive "
    "engaged'; 'a document keeps a traceable trail').\n"
...
CALL_B_SYSTEM_PROMPT = (
    ...
    "    input_slots: [{{\n"
    "      slot_key: str,            // free-form, human-readable\n"
    "      description: str,\n"
    "      required: bool,\n"
    ...
    "      slot_category: 'participant_availability' | "
    "'participant_email' | 'participant_phone' | 'meeting_time' | "
    "'attachment_document' | 'customer_decision' | "
    "'customer_data_share' | 'internal_artifact' | "
    "'internal_review' | 'external_query' | 'rep_metadata' | "
    "'due_date' | 'other',\n"
    "      // ``slot_category`` is REQUIRED. Pick the closest. "
    "Examples:\n"
    "      //   ``rajiv_availability`` -> participant_availability\n"
    "      //   ``it_director_email`` -> participant_email\n"
    "      //   ``meeting_time``      -> meeting_time\n"
    "      //   ``confirmed_slot``    -> meeting_time\n"
    "      //   ``roi_model``         -> internal_artifact\n"
    "      //   ``msa_redline``       -> internal_artifact\n"
    "      //   ``rfp_rubric``        -> customer_data_share\n"
    "      //   ``customer_pricing_choice`` -> customer_decision\n"
    "      //   ``rep_name``          -> rep_metadata\n"
    "      //   ``legal_sign_off``    -> internal_review\n"
    "      //   ``competitor_research`` -> external_query\n"
    "      //   only use ``other`` when none of the above fits.\n"
    "      critical: bool,           // True when the artifact body "
    "would be INCOHERENT or MISLEADING without this slot's value. "
    "False when the artifact has a graceful default OR can be "
    "rephrased to omit this reference without losing meaning.\n"
    "      // Examples of critical = True: a meeting-invite email "
    "that introduces ``Rajiv'' by name and explains he'll cover "
    "integration topics (without him, the paragraph is wrong); a "
    "CRM note referencing ``the agreed pricing tier'' (without it, "
    "the note misleads).\n"
    "      // Examples of critical = False: a sign-off line "
